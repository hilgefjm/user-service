...
function RegisterUser (userName, password, email, callback) {
  properties.get('login.encrpytionAlgorithm.blowfish', (err, algorithm) => {
    if (err) return callback(err);

    properties.get('login.sql.register', (err, statement) => {
      if (err) return callback(err);
...
function Authenticate (userName, password, callback) {
  properties.get('login.sql.authenticate', (err, statement) => {
