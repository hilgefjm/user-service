...
/* Respond hello NAME */
logger.info('Registering Status service at /status');
router.get('/:id', function (req, res) {
  var status = req.params.id;
  var msg = getHttpStatus(status);
  msg == HTTP_STATUS_CODE.HTTP_400 ? status = 400 : status=parseInt(status);

  res.status(status).json({ http_status: { status: status, message: msg } });
});
...
function getHttpStatus (code) {
  ...
  logger.debug('Code: ' + code + ' Message: ' + msg);
  return msg;
}
